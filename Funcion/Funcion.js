...
function mostrarResultadoMensaje(data, contenedorId) {
  ...
  const res = document.getElementById(contenedorId);
  res.style.display = 'block';
  res.style.animation = 'none';
  res.offsetHeight;
  res.style.animation = 'slideUp 0.35s ease both';
  res.innerHTML = `
...
const iconosPorExtension = {
  ...
  docx: '📘', doc: '📘', xlsx: '📗', apk: '📱',
  jpg: '🖼️', png: '🖼️', jpeg: '🖼️', mp4: '🎬', default: '📄'
};
...
function dragOver(e) {
  e.preventDefault();
  const zona = document.getElementById('zonaOCR').style.display === 'none'
    ? 'zonaDrop' : 'zonaDropOCR';
  document.getElementById(zona).classList.add('dragover');
}

function dragLeave(e) {
  document.getElementById('zonaDrop').classList.remove('dragover');
  document.getElementById('zonaDropOCR').classList.remove('dragover');
}
...
async function analizarArchivo() {
  ...
  res.style.display = 'none';
  label.textContent = 'Enviando archivo a VirusTotal...';
  bar.style.width = '0%';
...
function mostrarResultadoArchivo(data) {
  ...
  const titulo = esPeligroso ? '¡ARCHIVO PELIGROSO! No lo abras.'
    : esSospechoso ? '¡ARCHIVO SOSPECHOSO! Procede con cuidado.'
    : 'Archivo parece LIMPIO';

  res.style.display = 'block';
  res.style.background = bg;
  res.style.borderColor = borde;
  res.innerHTML = `
...
// ══════════════════════════════════════
// MODO TABS — ARCHIVO vs OCR
// ══════════════════════════════════════
function cambiarModo(modo) {
  const zonaArchivo = document.getElementById('zonaArchivoCompleta');
  const zonaOCR = document.getElementById('zonaOCR');
  const tabArchivo = document.getElementById('tabArchivo');
  const tabImagen = document.getElementById('tabImagen');

  if (modo === 'archivo') {
    zonaArchivo.style.display = 'block';
    zonaOCR.style.display = 'none';
    tabArchivo.classList.add('activo');
    tabImagen.classList.remove('activo');
  } else {
    zonaArchivo.style.display = 'none';
    zonaOCR.style.display = 'block';
    tabImagen.classList.add('activo');
    tabArchivo.classList.remove('activo');
  }
}

// ══════════════════════════════════════
// PANTALLA 2B — OCR IMÁGENES
// ══════════════════════════════════════
let imagenActual = null;

document.getElementById('inputImagenOCR').addEventListener('change', function(e) {
  if (e.target.files[0]) seleccionarImagenOCR(e.target.files[0]);
});

function dragDropOCR(e) {
  e.preventDefault();
  document.getElementById('zonaDropOCR').classList.remove('dragover');
  if (e.dataTransfer.files[0]) seleccionarImagenOCR(e.dataTransfer.files[0]);
}

function seleccionarImagenOCR(file) {
  imagenActual = file;
  const ext = file.name.split('.').pop().toLowerCase();
  const tamano = file.size < 1024 * 1024
    ? (file.size / 1024).toFixed(1) + ' KB'
  ...
  document.getElementById('imagenTamano').textContent = tamano + ' · ' + ext.toUpperCase();
  document.getElementById('imagenSeleccionada').style.display = 'flex';
  document.getElementById('zonaDropOCR').style.display = 'none';

  const btn = document.getElementById('btnAnalizarOCR');
  btn.disabled = false;
  btn.style.opacity = '1';
  btn.style.cursor = 'pointer';
  document.getElementById('resultado-ocr').style.display = 'none';
  document.getElementById('textoExtraido').style.display = 'none';
}

async function analizarImagenOCR() {
  if (!imagenActual) return;
  const res = document.getElementById('resultado-ocr');
  const textoBox = document.getElementById('textoExtraido');

  res.style.display = 'block';
  res.innerHTML = '<div class="cargando">🔍 Leyendo texto de la imagen con OCR...</div>';
  textoBox.style.display = 'none';

  try {
    const formData = new FormData();
    formData.append('archivo', imagenActual);
    const response = await fetch('/analizar-imagen', { method: 'POST', body: formData });
    const data = await response.json();

    if (data.error) {
      res.innerHTML = `<div class="cargando">⚠️ ${data.error}</div>`;
      return;
    }

    if (data.texto_extraido) {
      document.getElementById('textoExtraidoContenido').textContent = data.texto_extraido;
      textoBox.style.display = 'block';
    }

    mostrarResultadoMensaje(data, 'resultado-ocr');

  } catch (e) {
    res.innerHTML = '<div class="cargando">❌ Error al analizar la imagen.</div>';
  }
}

function limpiarOCR() {
  imagenActual = null;
  document.getElementById('zonaDropOCR').style.display = 'block';
  document.getElementById('imagenSeleccionada').style.display = 'none';
  document.getElementById('resultado-ocr').style.display = 'none';
  document.getElementById('textoExtraido').style.display = 'none';
  document.getElementById('inputImagenOCR').value = '';
  const btn = document.getElementById('btnAnalizarOCR');
  btn.disabled = true;
  btn.style.opacity = '0.5';
  btn.style.cursor = 'not-allowed';
